...
static void on_mesh_packet(const uint8_t *src_mac, const void *data,
  ...
  switch (hdr->type) {
  case MESH_PKT_HELLO:
    ESP_LOGI(LOG_TAG_DISCOVERY, "HELLO from node %u (" MACSTR ")", hdr->src_id,
             MAC2STR(src_mac));
    break;
...
void app_main(void) {
  ...
  audio_hal_speaker_init(); // We should decide a standard: Function do o do not
                            // return esp_err_t? For now, it just logs and
                            // continues.
  ESP_LOGI(TAG, "  [OK] I2S Amplifier ...... 16 kHz, 32-bit, Mono");

  xTaskCreatePinnedToCore(audio_hal_mic_read_task, "Mic_Read", 4096, NULL, 5,
                          NULL, 1);
  xTaskCreatePinnedToCore(afe_processing_task, "AFE_Proc", 8192, NULL, 5, NULL,
                          0);
  ...
  /* ── Footer ─────────────────────────────────────────────── */
  ESP_LOGI(TAG, "+------------------------------------------+");
  ESP_LOGI(TAG, "|      SYSTEM READY! Running v0.207        |");
  ESP_LOGI(TAG, "+------------------------------------------+");
}
