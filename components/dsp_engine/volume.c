...
void volume_init(volume_state_t *vol) {
	...
	vol->noise_floor =
		DEFAULT_NOISE_FLOOR; /* Suitable for DC-corrected MEMS mic */
}
...
void volume_calibrate(volume_state_t *vol, float measured_rms) {
	...
	if (vol->noise_floor < 200.0f)
		vol->noise_floor = 200.0f;
	if (vol->noise_floor > 4000.0f)
		vol->noise_floor = 4000.0f;
}
...
float volume_ramp(volume_state_t *vol, float target) {
	float coeff =
		(target > vol->current) ? vol->attack_coeff : vol->release_coeff;
